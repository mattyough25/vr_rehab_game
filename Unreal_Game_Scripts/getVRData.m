...
sFile = sprintf('OCULUS_HeadsetOrientationTest_%s.bin', datestr(now,'mm-dd-yyyy-HH-MM-SS')); %Automated naming of real bb task
...
if bRun
    ...
    bWaitbar = waitbar(0, 'Getting Kinematic Data', 'Name', 'Collecting Data','CreateCancelBtn','delete(gcbf)');
    tic
    while bRun
    drawnow;

        %% read unreal data
        nData = matlab_client('request_data');
...
%         end
        ...
        if par.bWrite2File
            save_the_data(par, tTime, nData)

        end

        if par.bDF
            send_to_DF(par, tTime,  nData)
        end
